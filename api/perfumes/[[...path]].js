/**
 * Uma única serverless function para /api/perfumes e /api/perfumes/:id
 * path undefined ou [] -> lista (GET) ou cria (POST)
 * path = [id] -> GET/PUT/DELETE por id
 */
...
async function handleList(req, res) {
  ...
  try {
    if (catalog && ["arabe", "feminino", "normal"].includes(catalog)) {
      rows = onlyActive
        ? await sql`SELECT p.id, p.external_url, p.title, p.description, p.catalog_source, p.notes, p.variants, p.image_2_url, p.ativo, p.esgotado FROM perfumes p WHERE p.catalog_source = ${catalog} AND COALESCE(p.ativo, true) = true ORDER BY p.title`
        : await sql`SELECT p.id, p.external_url, p.title, p.description, p.catalog_source, p.notes, p.variants, p.image_2_url, p.ativo, p.esgotado FROM perfumes p WHERE p.catalog_source = ${catalog} ORDER BY p.title`;
    } else {
      rows = onlyActive
        ? await sql`SELECT id, external_url, title, description, catalog_source, notes, variants, image_2_url, ativo, esgotado FROM perfumes WHERE COALESCE(ativo, true) = true ORDER BY catalog_source, title`
        : await sql`SELECT id, external_url, title, description, catalog_source, notes, variants, image_2_url, ativo, esgotado FROM perfumes ORDER BY catalog_source, title`;
    }
  } catch (queryErr) {
    if (queryErr?.code !== "42703") throw queryErr;
    rows = catalog && ["arabe", "feminino", "normal"].includes(catalog)
      ? await sql`SELECT p.id, p.external_url, p.title, p.description, p.catalog_source, p.notes, p.variants, p.image_2_url FROM perfumes p WHERE p.catalog_source = ${catalog} ORDER BY p.title`
      : await sql`SELECT id, external_url, title, description, catalog_source, notes, variants, image_2_url FROM perfumes ORDER BY catalog_source, title`;
  }
  ...
  const imagesRows = idSet.size ? allImages.filter((r) => idSet.has(toUuidKey(r.perfume_id ?? r.perfumeId))) : [];
...
async function handlePostCreate(req, res) {
  ...
  if (!["arabe", "feminino", "normal"].includes(catalogSource)) return res.status(400).json({ error: "Catálogo deve ser: arabe, feminino ou normal" });
...
async function handleGetOne(id, req, res) {
  let p;
  try {
    [p] = await sql`SELECT id, external_url, title, description, catalog_source, notes, variants, image_2_url, ativo, esgotado FROM perfumes WHERE id = ${id}`;
  } catch (queryErr) {
    if (queryErr?.code !== "42703") throw queryErr;
    [p] = await sql`SELECT id, external_url, title, description, catalog_source, notes, variants, image_2_url FROM perfumes WHERE id = ${id}`;
  }
...
async function handlePut(id, req, res) {
  ...
  await sql`UPDATE perfumes SET external_url = ${externalUrl}, title = ${title}, description = ${description}, catalog_source = ${catalogSource}, notes = ${JSON.stringify(notes)}, variants = ${JSON.stringify(variants)}, ativo = ${ativo}, esgotado = ${esgotado}, updated_at = now() WHERE id = ${id}`;
